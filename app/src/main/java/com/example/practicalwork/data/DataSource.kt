package com.example.practicalwork.data

import com.example.practicalwork.R
import com.example.practicalwork.model.Courses


object DataSource {
        val course = listOf(
            Courses(R.string.architecture, 58, R.drawable.architecture),
            Courses(R.string.automotive, 30, R.drawable.automotive),
            Courses(R.string.biology, 90, R.drawable.biology),
            Courses(R.string.crafts, 121, R.drawable.crafts),
            Courses(R.string.business, 78, R.drawable.business),
            Courses(R.string.culinary, 118, R.drawable.culinary),
            Courses(R.string.design, 423, R.drawable.design),
            Courses(R.string.ecology, 28, R.drawable.ecology),
            Courses(R.string.engineering, 67, R.drawable.engineering),
            Courses(R.string.fashion, 92, R.drawable.fashion),
            Courses(R.string.finance, 100, R.drawable.finance),
            Courses(R.string.film, 165, R.drawable.film),
            Courses(R.string.gaming, 37, R.drawable.gaming),
            Courses(R.string.geology, 290, R.drawable.geology),
            Courses(R.string.drawing, 326, R.drawable.drawing),
            Courses(R.string.history, 189, R.drawable.history),
            Courses(R.string.journalism, 96, R.drawable.journalism),
            Courses(R.string.law, 58, R.drawable.law),
            Courses(R.string.lifestyle, 305, R.drawable.lifestyle),
            Courses(R.string.music, 212, R.drawable.music),
            Courses(R.string.painting, 172, R.drawable.painting),
            Courses(R.string.photography, 321, R.drawable.photography),
            Courses(R.string.physics, 41, R.drawable.physics),
            Courses(R.string.tech, 118, R.drawable.tech),
        )
}
